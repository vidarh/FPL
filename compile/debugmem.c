#ifdef RCS
static char rcsid[] = "$Id: debugmem.c,v 1.1 1996/01/24 19:56:40 Daniel Exp $";
#endif
/******************************************************************************
 ...
 *
 * Project: FR-3000 CS
 * $Source: Lazer:Codingx/C/src/FPL/compile/RCS/debugmem.c,v $
 * $Revision: 1.1 $
	 ...
	 int		line_I;
{
	...
	if( !mem_P) {
		NULLFREEACTION(source_PC, line_I);
		return;
	}
	debug_PS = (struct DebugMem *)
...
/*************************************************************************
 *
 * Function   : DBG_MemListCheck
 * Purpose    : Check the cookies around ALL memory allocations for
 *              overwritten memory areas!
 * Parameters : source file, source line
 * Returns    : 0 if OK
 *
 *************************************************************************/
...
long DBG_MemListCheck(char *file, long line)
{
	...
	struct 	DebugMem *point = MallocKey;
	do {
		char *tempmem=((char *) point + sizeof( struct DebugMem ) + PRE_COOKIE_SIZE);
		ret+=DBG__CheckMem(tempmem, file, line);
		point = point->prev;
	} while (point && !ret);
	return ret;
}
